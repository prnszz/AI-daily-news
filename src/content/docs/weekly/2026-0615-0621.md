---
title: "Weekly Digest - 2026/06/15〜06/21"
...
CloudflareのTemporary Accountsは、エージェントが事前登録なしにWorkerをデプロイし、あとから人間が正式アカウントへ引き継げる仕組みを提供した。VercelのConnectは、長期間有効なシークレットを保持せずに、スコープを絞った短期トークンで外部サービスへ接続する仕組みをパブリックベータで公開した。いずれも、本番でエージェントを動かすときの登録・認証・シークレット管理の摩擦を減らす方向だ。
